...
if (
  !manifest.repository?.url?.endsWith(
    "/vega-plugin-richtext-markdown.git",
  )
) {
  throw new Error(
    "package repository must point to the independent Markdown plugin repository",
  );
}
if (
...
if (
  !manifest.dependencies?.["@haneoka/vega-plugin-richtext-html"]
) {
  throw new Error(
    "the audited HTML sanitizer package must remain a runtime dependency",
  );
}
if (
  manifest.peerDependencies?.["@haneoka/vega-plugin-richtext"] !== "^0.1.0"
) {
